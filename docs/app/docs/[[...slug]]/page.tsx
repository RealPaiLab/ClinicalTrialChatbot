...
function lastUpdated(page: { path: string; data: { lastModified?: Date } }): Date | undefined {
  ...
  try {
    return statSync(join(process.cwd(), 'content/docs', page.path)).mtime;
  } catch {
    return undefined;
  }
}
...
export default async function Page(props: PageProps<'/docs/[[...slug]]'>) {
  ...
  const updated = lastUpdated(page);

  return (
    <DocsPage toc={page.data.toc} full={page.data.full}>
      <DocsTitle>{page.data.title}</DocsTitle>
      <DocsDescription className="mb-0">{page.data.description}</DocsDescription>
      <div className="flex flex-row items-center gap-2 border-b pb-6 [&_button]:font-semibold">
      ...
      {updated ? <PageLastUpdate date={updated} /> : null}
    </DocsPage>
  );
}
