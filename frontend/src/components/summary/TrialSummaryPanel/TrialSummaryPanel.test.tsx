...
describe('TrialSummaryPanel', () => {
  it('shows the empty state when no trial is selected', () => {
    renderWithClient(<TrialSummaryPanel trial={null} />);
  ...
  it('renders the official title, facts, description, and criteria', () => {
    renderWithClient(<TrialSummaryPanel trial={mockTrials[0]} />);
    expect(
  ...
  it('adds the trial to context when the AI button is clicked', async () => {
    ...
    renderWithClient(<TrialSummaryPanel trial={mockTrials[0]} onAddToContext={onAddToContext} />);
  ...
  it('disables the AI button once the trial is in context', () => {
    renderWithClient(
      <TrialSummaryPanel trial={mockTrials[0]} onAddToContext={vi.fn()} isInContext />
    );
